...
func main() {
	...
	for scanner.Scan() {
		...
		if index == 5 {
			...
			turnsToWin, score := getBingoTurnsToWinAndScore(bingoBoard[:], bingoNumbers, mostTurnsToWin)
			if turnsToWin != -1 && mostTurnsToWin == -1 || mostTurnsToWin < turnsToWin || (mostTurnsToWin == turnsToWin && score > winningBoardScore) {
				mostTurnsToWin = turnsToWin
