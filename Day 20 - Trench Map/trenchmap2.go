...
func calculateNumberofLitPixels(enhancedImage [][]string, imageEnhancementAlgorithm []string, steps int) int {
	...
	litPixelCount := 0
	notCountedPixels := overallPadding - steps
...
func main() {
	...
	for scanner.Scan() {
		...
		if isFirstImageRow {
			...
			for i := 0; i < overallPadding; i++ {
				imagePadding := []string{}
				for pixel := 0; pixel < imageSize+(overallPadding*2); pixel++ {
		...
		imageRow := stringToListWithPadding(scanner.Text(), overallPadding)
	...
	fmt.Printf("The number of lit pixels is: %d\n", calculateNumberofLitPixels(enhancedImage, imageEnhancementAlgorithm, 50))
}
