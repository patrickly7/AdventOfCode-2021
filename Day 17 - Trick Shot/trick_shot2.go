...
func calculateHighestYPositionAndDistinctVelocities(xBounds []int, yBounds []int) (int, int) {
	highestYPosition := 0
	distinctVelocities := 0
	...
	for x := 0; x < 1000; x++ {
		for y := -1000; y < 1000; y++ {
			...
			for loops < 1000 {
				...
				if currPosition[0] >= xBounds[0] && currPosition[0] <= xBounds[1] && currPosition[1] >= yBounds[0] && currPosition[1] <= yBounds[1] {
					distinctVelocities++
					fmt.Printf("%d, %d with Pos (%d, %d)\n", x, y, currPosition[0], currPosition[1])
	...
	return highestYPosition, distinctVelocities
}
...
func main() {
	...
	yPos, velocities := calculateHighestYPositionAndDistinctVelocities(xBoundaries, yBoundaries)
	fmt.Printf("Highest Y Position is: %d, %d Velocities Hit the Target Area \n", yPos, velocities)
}
